function LevelCreator(gameController) {
  ...
  function generatePlanetLocations(planetDensity, cb) {
    ...
    var planetLocations = [];
    var numberOfPlanets = (planetDensity * vc.getLevelHeightMinusMargins() *
        vc.getLevelWidthMinusMargins());
  ...
  function generatePlanetLocation(planetLocations, cb) {
    ...
    var margin = gc.getViewController().getLevelMargin();

    var newPlanetLocation = {
      x: getRandomInt(margin, maxX - margin),
      y: getRandomInt(margin, maxY - margin)
    };
  ...
  function getProbability(newPlanetLocation, planetLocations) {
    ...
    // Calculate probability based on other planets.
